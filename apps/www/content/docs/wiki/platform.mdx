...
| `power`   | `caffeinate` / `systemd-inhibit` | PowerShell `SetThreadExecutionState`            | Keeping the system awake while remote access is on. A helper child holds the request and exits with its owner; Linux also inhibits the lid switch.                                                     |
