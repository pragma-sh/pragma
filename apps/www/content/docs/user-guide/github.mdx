...
- **Changed files** — read every file as a side-by-side diff against the base, with a
  per-file done-toggle. Drag the bar under a diff to make it taller or shorter, or
  double-click the bar to fit the whole file. The page scrolls past a diff until you click
  into it.
- **Review threads** — reply inline in markdown, resolve and unresolve optimistically,
  right from the app. The toolbar's up/down arrows (or `Cmd/Ctrl+↑/↓`) step through
  comments. Each step centers the comment in its diff and the diff on screen.
