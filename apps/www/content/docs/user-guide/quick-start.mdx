...
- **Local** — **Add project** opens a folder from disk, or paste a remote URL and
  **Clone remote** into Pragma's projects directory.
- **Remote** — connect a project over SSH (see [SSH support](/docs/user-guide/ssh)).

<Callout title="Folders without git">
  Pragma drives real git worktrees, so a git checkout gets every feature. If the folder you pick is
  not a git repository, Pragma warns you and offers to **Initialize git repository**, **Open another
  project**, **Continue without git**, or **Don’t show again** (turn the warning back on under
  Settings → Other). A project without git shows a single **root** worktree: terminals, tabs, and
  agents work as usual, but new worktrees, Changes, and Pull Request stay off until you initialize a
  repository — which you can do later from the new-worktree button or either pane. Initializing
  needs a git commit identity (`user.name` / `user.email`). Deleting from the Files pane asks first
  in a folder without git, since there is no git history to restore the file from.
</Callout>
