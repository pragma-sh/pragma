...
remote URL, or connected over SSH. A local folder that is not a git repository can be added
too: it has a single **root** worktree and no git features until you initialize a repository,
which keeps its tabs and agent sessions. Projects live in the **project switcher** strip at the
