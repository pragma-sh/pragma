...
export function usePushNotifications(): void {
  ...
  useEffect(() => {
    // No notifications on web means no taps to route from, and the listener
    // APIs have no browser implementation to subscribe to.
    if (status !== "paired" || Platform.OS === "web") return;
