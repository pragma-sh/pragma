...
async function registerToken(client: PragmaClient, signal: AbortSignal): Promise<PushRegistration> {
  // Expo push is a native service. A browser tab would need Web Push with its
  // own VAPID keys and a service worker, which the gateway does not speak, so
  // the web build reports the same "unsupported" a simulator does.
