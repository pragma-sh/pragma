# `@pragma-sh/watcher` - Watcher Sidecar

Host-side Bun sidecar compiled as `pragma-watch`. It loads plugin `defineWatcher`
contributions, attaches them to live PTY sessions through `@pragma-sh/sdk`, streams output,
...
- Keep transport through `@pragma-sh/sdk`; do not hand-build HTTP routes.
